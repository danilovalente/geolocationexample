...
var TransportRepo TransportRepository
...
func (repo *TransportRepository) Get(id string) (*domain.Transport, error) {
	collection := repo.Conn.Database("geolocation").Collection("transports")
...
func init() {
	TransportRepo = TransportRepository{Conn: DBClient.Conn}
}
